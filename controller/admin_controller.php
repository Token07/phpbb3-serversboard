...
class admin_controller
{
    ...
    public function add_server()
    {
        ...
        if ($this->request->is_set_post('submit')) {
            ...
            $this->phpbb_log->add('admin', $this->user->data['user_id'], $this->user->data['session_ip'], 'TOKEN07_SERVERSBOARD_ACP_LOG_ADDED', time(), [$server_ip]);
    ...
    public function delete_server()
    {
        if (confirm_box(true)) {
            ...
            if ($row = $this->db->sql_fetchrow($result)) {
                ...
                $this->phpbb_log->add('admin', $this->user->data['user_id'], $this->user->data['session_ip'], 'TOKEN07_SERVERSBOARD_ACP_LOG_DELETE', time(), [$row['server_ip']]);
    ...
    public function move_server($action)
    {
        ...
        if ($this->request->is_ajax()) {
            ...
            $json_response->send(['success' => $status]);
    ...
    public function edit_server($server_id)
    {
        if ($this->request->is_set_post('submit')) {
            ...
            $this->db->sql_query($sql);
            $this->phpbb_log->add('admin', $this->user->data['user_id'], $this->user->data['session_ip'], 'TOKEN07_SERVERSBOARD_ACP_LOG_UPDATED', time(), [$server_ip]);
    ...
    public function protocol_test()
    {
        if ($this->request->is_set_post('submit')) {
            ...
            foreach ($results["$server_ip:$server_port"] as $k => $v) {
                ...
                $this->template->assign_block_vars('serverdata', [
                    'KEY'          => $k,
    ...
    private function generate_protocol_list()
    {
        ...
        $curProto = '';
        $baseProtos = [];
        $sortExceptions = ['bf3', 'quake3', 'samp', 'ase', 'starmade', 'lhmp']; // Servers that are actually games
        foreach ($protocols as $protocol) {
            if (($protocol['short'] != $protocol['protocol']) || ($protocol['short'] == $protocol['protocol'] && in_array($protocol['short'], $sortExceptions))) {
                if ($protocol['protocol'] != $curProto) {
                    ...
                    $this->template->assign_block_vars('serversboard_base_protocols', [
                        'CATEGORY'    => $protocol['protocol'],
                    ]);
                }
                $this->template->assign_block_vars('serversboard_base_protocols.protocols', [
                    'NAME'         => $protocol['name'],
    ...
    private function move($id, $delta)
    {
        ...
        $sql = 'SELECT server_order FROM '.$this->serversboard_table.' WHERE server_id = '.(int) $id;
